...
emprestimos_biblioteca.value_counts
emprestimos_biblioteca.head() #retorna as linhas/séries do Dataframe.

#Importando mais dados do dados_exemplares.parquet
...
#Concatena novamente os Dataframes
emprestimos_completo = emprestimos_biblioteca.merge(dados_exemplares)
emprestimos_completo

#Marco - Day 2

emprestimos_completo.head() #retorna as linhas/séries do Dataframe.

#Cria uma lista com base na coluna com nomenclatura da 'localizacao' do endereço 'emprestimos_completo'
CDU_lista=[]

for CDU in emprestimos_completo['localizacao']:
    if (CDU < 100): #Se o CDU for menor que 100, colocar na lista de nome: Generalidades
...
#Lista CDU_geral, recebe os dados da CDU_lista.
emprestimos_completo['CDU_geral'] = CDU_lista 

#Exclusão de coluna
emprestimos_completo.drop(columns=['registro_sistema'],inplace=True, errors='ignore')

#Muda o tipo de dado da matrícula
emprestimos_completo['matricula_ou_siape']=emprestimos_completo['matricula_ou_siape'].astype('string')

#Visualiza o dataframe final
print(emprestimos_completo.head()) #retorna as linhas/séries do Dataframe.
print(len(emprestimos_completo))
#finalizado